...
def find_transcode_candidates(api, limit=None, offset=None):
    print("Searching for transcode candidates...")
    return api.seeding(limit=limit, offset=offset)


def get_transcode_candidates(api, limit=None, offset=None):
    candidates = find_transcode_candidates(api, limit=limit, offset=offset)
    results = []
    for group_id, torrent_id in candidates:
        group = api.torrent_group(group_id)
        if group is not None:
            torrent = next(
                filter(lambda t: t["id"] == torrent_id, group["torrents"]))
...
def find_and_upload_missing_transcodes(candidates, api, upload_torrent, single, add_to_qbittorrent):
    ...
    for groupId, torrentId in candidates:
        ...
        if group is not None:
            torrent = next(
                filter(lambda t: t["id"] == torrentId, group["torrents"]))
            ...
            release_url = "Release URL      : %s" % api.release_url(
                groupId, torrentId)
            print("\n\n")
            ...
            if not torrent["filePath"]:
                print("No filepath for %s (%s) - skipping" %
            ...
            for data_dir in data_dirs:
                flac_dir = os.path.join(
                    data_dir, html.unescape(torrent["filePath"]))
                if os.path.exists(flac_dir):
                    data_dir_found = True
                    break

            if not data_dir_found:
                print("Path (%s) does not exist in data directories - skipping" %
                      torrent["filePath"])
                continue
            ...
            for format in needed:
                ...
                try:
                    if len(torrent['remasterTitle']) >= 1:
                        basename = group["artist"] + " - " + group["name"] + \
                            " (" + torrent['remasterTitle'] + ") " + "[" + \
                            group["year"] + "] (" + torrent['media'] + " - "
                    else:
                        basename = group["artist"] + " - " + group["name"] + \
                            " [" + group["year"] + \
                            "] (" + torrent['media'] + " - "
                    ...
                    transcode_dir = transcode.transcode_release(
                        flac_dir, output_dir, basename, format)
                    if not transcode_dir:
                        print(
                            "Some file(s) in this release were incorrectly marked as 24bit - skipping")
                        break

                    new_torrent = transcode.make_torrent(
                        transcode_dir, tmpdir, api.announce_url)
                    permalink = api.permalink(torrent["id"])
                    description = create_description(
                        flac_dir, format, permalink)
                    result = api.upload(
                        group, torrent, new_torrent, format, description, not upload_torrent)
                    pprint(result.text)
                    ...
                    if add_to_qbittorrent and config.get_qbittorrent_config():
                        ...
                        if qbt_client.connect():
                            qbt_client.add_torrent(
                                new_torrent_dest, transcode_dir)
                    ...
                    print("Error adding format %s: %s: %s" %
