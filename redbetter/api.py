...
class AuthenticationError(GazelleAPIError):
    pass
...
class GazelleAPI:
    ...
    def upload(self, group, torrent, new_torrent, format, description, dry_run):
        ...
        with open(new_torrent, "rb") as torrent_file:
            ...
            response = self.__post(self.api_url, params={
                                   'action': "upload"}, data=form, files=files)
    ...
    def __get_announce_url(self):
        try:
            ...
            handle_error(description=f"Authentication to {self.sitename} failed", exception_details=str(
                e), should_raise=True)
