...
def test_get_candidates_red(client):
    with patch('redbetter.webserver.get_transcode_candidates') as mock_get_candidates:
        ...
        mock_get_candidates.assert_called_once_with(app.config['red_api'], set(), limit=None, offset=None)
...
def test_get_candidates_ops(client):
    with patch('redbetter.webserver.get_transcode_candidates') as mock_get_candidates:
        ...
        mock_get_candidates.assert_called_once_with(app.config['ops_api'], set(), limit=None, offset=None)


def test_get_candidates_all(client):
    with patch('redbetter.webserver.get_transcode_candidates') as mock_get_candidates:
        mock_get_candidates.side_effect = [[{'name': 'red_candidate'}], [{'name': 'ops_candidate'}]]
...
def test_get_candidates_with_pagination(client):
    with patch('redbetter.webserver.get_transcode_candidates') as mock_get_candidates:
        ...
        assert response.status_code == 200
        mock_get_candidates.assert_called_once_with(app.config['red_api'], set(), limit=10, offset=5)
